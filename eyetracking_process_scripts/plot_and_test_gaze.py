...
if vid == 'despicable_me_english':
   old_data_patients = []#'NS127_02','NS135','NS136','NS137','NS138','NS140','NS140_02','NS145','NS151','NS151_02','NS153','NS154','NS164','NS166','NS174_02','NS174_03']
   # new_data_patients = ['NS155_02','NS178','NS189_03','NS190','NS191','NS193','NS194','NS201_02','NS203','NS204','NS205','NS206']
   new_data_patients = ['NS190','NS191','NS192','NS193']
if vid == 'inscapes':
...
#             if any(k.lower() for k in keys in filename:
...
for i, pat in enumerate(patients):
    ...
    et_files = [
        f for f in os.listdir(eye_prep_dir) if f.endswith('.npz')
        if f.endswith('.npz')
        and any(k.lower() in f.lower() for k in keys)
        and not f.startswith('._')
    ]
    
   # if length(et_files) > 1:
       # find some way to match 
    et_file = et_files[0]  # Take the first matching file
  ...
  #      continue
    ...
    print(f'Loading data {et_file} for patient {pat} ...')
...
if compute_ISC:
    ...
    for i, pat in enumerate(patients):
        ...
        et_files = [
            f for f in os.listdir(eye_prep_dir)
            if f.endswith('.npz')
            and any(k.lower() in f.lower() for k in keys)
            and not f.startswith('._')
        ]
        ...
        if not et_files:
            print(f"No matching .npz files found for patient {pat} and keys={keys}")
            continue
    ...
    if not xy:
        raise RuntimeError("No valid eye data loaded for any patient; check file matching / directory structure.")
    ...
    isc_gaze_perm = np.empty((n_perm, gaze_var.shape[1]))  # (#perms, #subs)
    
    for p in range(n_perm):
        corr = np.corrcoef(gaze_var_perm[:, p, :].T)  # (subs x subs)
        corr[np.eye(corr.shape[0]) == 1] = np.nan
        isc_gaze_perm[p, :] = np.nanmean(corr, axis=0)
            
    #%% Slow fluctuations 
    fs_eye_ds = fs_eye / dsf
    sos = signal.butter(5, [0.05, 0.15], btype='bandpass', fs=fs_eye_ds, output='sos')
    ...
    isc_time_var, _ = time_resolved_isc(gaze_var, (fs_eye/dsf), window, overlap)
    isc_time_var_perm = np.empty(np.hstack((isc_time_var.shape, n_perm)))
