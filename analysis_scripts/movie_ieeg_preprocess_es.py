...
target_patient = 'NS219'#'NS205'  # e.g., 'NS189' or 'NS189_01'
#target_movie = 'sub-NS127_ses-02_task-the_present_run-1_ieeg.nwb'
target_movie = 'NS219_ses-Exp_Samp_Betta01_behavior+ecephys.nwb'
...
# %%
## saveaudio if itexists
            import numpy as np
            from scipy.io.wavfile import write
            
            audio_ts = nwb.acquisition["audio"]
            
            fs = int(round(audio_ts.rate))   # WAV needs an integer sample rate
            audio_data = np.asarray(audio_ts.data[:])  # shape: samples x channels
            
            # Normalize safely to [-1, 1]
            max_abs = np.max(np.abs(audio_data))
            if max_abs > 0:
                audio_norm = audio_data / max_abs
            else:
            ...
            out_path = "nwb_audio_export.wav"
            
            # Define audio outpath
            audio_file_path = file_paths['bad_channels_file'].replace('_bad_channels.txt', '_audio_export.wav')
            
    
            write(audio_file_path, fs, audio_int16)
            
            print(f"Saved audio to: {out_path}")
            print(f"Original NWB rate: {audio_ts.rate}")
            print(f"WAV export rate: {fs}")

## save mic if it exists
            mic_ts = nwb.acquisition["mic"]
            
            fs = int(round(mic_ts.rate))   # WAV needs an integer sample rate
            mic_data = np.asarray(mic_ts.data[:])  # shape: samples x channels
            
            # Normalize safely to [-1, 1]
            max_abs = np.max(np.abs(mic_data))
            if max_abs > 0:
                mic_norm = mic_data / max_abs
            else:
                mic_norm = mic_data
            
            # Convert to 16-bit PCM for standard WAV compatibility
            mic_int16 = (mic_norm * 32767).astype(np.int16)
            
            out_path = "nwb_mic_export.wav"
            
            # Define mic outpath
            mic_file_path = file_paths['bad_channels_file'].replace('_bad_channels.txt', '_mic_export.wav')
            
    
            write(mic_file_path, fs, mic_int16)
            
            print(f"Saved mic to: {out_path}")
            print(f"Original NWB rate: {mic_ts.rate}")
            print(f"WAV export rate: {fs}")
...
#%%
            ...
            if pat == "NS217" and vid == "inscapes":
                first_valid_probe = 140
            if pat == "NS219" and vid == "Betta":
                first_valid_probe = 326
