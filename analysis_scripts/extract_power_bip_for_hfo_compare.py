...
vids = ['the_present']#,'inscapes']#,'despicable_me_english']#,'despicable_me_english']
...
ref_dir = '/Volumes/Samsung/akash_data/data-share'
refpair_dir = '/Volumes/Samsung/ref_pairs'
...
elec_method = 'cc'
include_cort_elecs = True

wd = '/Volumes/Samsung/scripts/movies_ET_attn_main'
src_dir = os.path.join(wd, 'src')
src_dir = os.path.abspath(src_dir)

# Fixed parameters (match MATLAB script)
params = {"fs":512,    # target sampling rate
          }
...
for vid in vids:
    ...
    for freq_band in freq_bands:
        ...
        for pat in patients:
            ...
            ### PASTING THIS IN, NEED TO RUN BIPOLAR REFERENCING THE WAY WE ARE DOING IT IN THE OTHER SCRIPT
            ## LFP DIR IS THE SAME AS FIF FIR I THINK
            from pathlib import Path
            
            linkage_path = "/Volumes/Samsung/akash_data/subjlist_linkage.xlsx"
            
            # Read oldID/newID linkage file
            linkage = pd.read_excel(linkage_path)
            
            # Make sure IDs are strings and remove accidental whitespace
            linkage["oldID"] = linkage["oldID"].astype(str).str.strip()
            linkage["newID"] = linkage["newID"].astype(str).str.strip()
            
            # Dictionary: NS127_02 -> P1, etc.
            old_to_new = dict(zip(linkage["oldID"], linkage["newID"]))
            
            for pat in patients:
                
                sub_fs_dir = f'{fs_dir}/{pat}'
                fif_dir    = f'{data_dir}/{pat}/Neural_prep'
                fif_files  = sorted(os.listdir(fif_dir), key=_sort_key)
                fif_files = [
                    f for f in fif_files
                    if f.endswith(".fif")
                    and 'aic' not in f
                ...
                print(f"\nFound {len(fif_files)} run(s) for {pat}:")
                for f in fif_files:
                    print("  ", f)
            
                for fif_file in fif_files:
            
                    ses_label = _extract_ses_label(fif_file)
                    run_label = _extract_run_label(fif_file)
                    entry_id  = f"{pat}_{ses_label}_{run_label}" if ses_label else f"{pat}_{run_label}"
                    print(f"\nProcessing {entry_id} ...")
            
                    fif_path = f'{fif_dir}/{fif_file}'
                    outdir   = f'{basedir}/{pat}'
                    os.makedirs(outdir, exist_ok=True)
            
                    # 1. Load and resample to 512 Hz (to align with the existing MATLAB script exactly)
                    ecogPreproc = mne.io.read_raw_fif(fif_path, preload=True)
            
                    if ecogPreproc.info['sfreq'] != params['fs']:
                        ecogPreproc.resample(params['fs'])
            
                    Fs   = ecogPreproc.info['sfreq']
                    time = ecogPreproc.times
                    # print(f"Sampling rate: {Fs} Hz  |  Duration: {time[-1]:.1f} s")
            
                    # 2. Electrode correspondence → GM / WM labels for bp referencing
                    # Load pre-computed electrode pairings (from subf_findelecwm.py)
                    
                    if elec_method == 'cc':
                        analysiselecs_path = os.path.join(refpair_dir, pat, 'analysiselecs_AMY.npz')
                        analysiselecs = np.load(analysiselecs_path, allow_pickle=True)
                        
                    elif elec_method == 'akash':
                            
                        ## USING AKASH'S BP PAIRS
                        pat_clean = str(pat).strip()
                    
                        # Lookup corresponding newID
                        if pat_clean not in old_to_new:
                            print(f"[WARNING] No newID found for {pat_clean}; skipping")
                            continue
                        
                        p_pat = old_to_new[pat_clean]
                        
                        pat_ref_dir = f"{ref_dir}/{p_pat}/mat"
                        ref_files  = sorted(os.listdir(pat_ref_dir))
                        ref_files = [
                            f for f in ref_files
                            if f.endswith(".npz")
                            and 'analysiselecs' in f
                            and not f.startswith("._")
                            and not f.startswith("~")
                        ]
                        
                        analysiselecs_path = os.path.join(pat_ref_dir, ref_files[0])
                        analysiselecs = np.load(analysiselecs_path, allow_pickle=True)
                                
                    
                    # Select amy and/or cortical contacts
                   # amyelecs  = analysiselecs['amyelecs'].item()
                    amyelecs = analysiselecs['amyelecs'].item()
                    cortelecs = analysiselecs['cortelecs'].item()
                    
                    
                    if elec_method == 'cc':
                        if include_cort_elecs:
                            gm_labels_all  = amyelecs['targetelec']  + cortelecs['targetelec']
                            ref_labels_all = amyelecs['wmelec']      + cortelecs['wmelec']
                            bp_names_all   = amyelecs['electrodes']  + cortelecs['electrodes']
                        else:
                            # gm_labels_all  = amyelecs['targetelec']
                            # ref_labels_all = amyelecs['wmelec'] 
                            # bp_names_all   = amyelecs['electrodes']
                            amy_labels_all  = amyelecs['targetelec']
                            amy_labels_all = amyelecs['wmelec'] 
                            amy_names_all   = amyelecs['electrodes']
                            
                    elif elec_method == 'akash':
                        if include_cort_elecs:
                            gm_labels_all = (
                                np.asarray(amyelecs["targetelec"], dtype=object).ravel().tolist()
                                + np.asarray(cortelecs["targetelec"], dtype=object).ravel().tolist()
                            )
                        
                            ref_labels_all = (
                                np.asarray(amyelecs["wmelec"], dtype=object).ravel().tolist()
                                + np.asarray(cortelecs["wmelec"], dtype=object).ravel().tolist()
                            )
                        
                            bp_names_all = (
                                np.asarray(amyelecs["electrodes"], dtype=object).ravel().tolist()
                                + np.asarray(cortelecs["electrodes"], dtype=object).ravel().tolist()
                            )
                        
                        else:
                            gm_labels_all = np.asarray(amyelecs["targetelec"], dtype=object).ravel().tolist()
                            ref_labels_all = np.asarray(amyelecs["wmelec"], dtype=object).ravel().tolist()
                            bp_names_all = np.asarray(amyelecs["electrodes"], dtype=object).ravel().tolist()
                    
                    
                    # Deduplicate — keep first occurrence of each pair
                    seen = set()
                    gm_labels, refs, bp_names = [], [], []
                    for g, r, b in zip(gm_labels_all, ref_labels_all, bp_names_all):
                        if b not in seen:
                            seen.add(b)
                            gm_labels.append(g)
                            refs.append(r)
                    ...
                    # Remove pairs where either electrode is bad
                    # bads = set(ecogPreproc.info.get('bads', []))
                    # valid = [(g, r, b) for g, r, b in zip(gm_labels, refs, bp_names)
                    #          if g not in bads and r not in bads]
                    # gm_labels = [v[0] for v in valid]
                    # refs      = [v[1] for v in valid]
                    # bp_names  = [v[2] for v in valid]
                    
                    # print(f"After dedup and bad channel removal: {len(bp_names)} pairs")
            
                    # 3. Bipolar re-reference
                    bp_raw = mne.set_bipolar_reference(
                        ecogPreproc,
                        anode=list(gm_labels), cathode=list(refs),
                        ch_name=bp_names, drop_refs=True, copy=True
                    ).pick(bp_names)
                    bp_raw.load_data()
                    
                    ### SAVE BP_RAW INTO LFP/FIF DIR WITH UNIQUE NAME
                    ## PROCEED TO EXTRACT POWER
            ...
            for lfp_file in lfp_files:
               ...
               # ip_contacts = elecs_subs.Contact.values[(elecs_subs.AparcAseg_Atlas != 'Right-Cerebral-White-Matter') & (elecs_subs.AparcAseg_Atlas != 'Left-Cerebral-White-Matter')]
                ...
                if len(ip_contacts) == 0:
                    ...
                    if find_peaks:
                        fooof_fig_dir = f'/{machine_path}/Samsung/Movie_data/fooof_peaks_{freq_band}_{vid}_26Mar26'
...
#%%
                ...
                for label in df.columns:
                    ...
                    # Find the matching row in elecs_subs based on Contact column
                    match_row = elecs_subs[elecs_subs['label'] == label]
                
                    if not match_row.empty:
                       # dk_regions.append(match_row['DK_Atlas'].values[0])  
