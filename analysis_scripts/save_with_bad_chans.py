...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 25 08:37:40 2026
...
#    'NS135',
 ...
 #   'NS151',
  #  'NS153',
 #   'NS154', # battery dead in middle of recording, not worth trying to save
 #   'NS155',
    # 'NS164',
    # 'NS166',
    #'NS178',
    ...
    for imp in implants_to_process:
        ...
        for mov in movies:
            ...
            for ref in ref_types:
                ...
                preprocessed_dir = sub_neural_prep_dir
                
                run_label = _extract_run_label(mov)
                ...
                fif_files = sorted([
                    ...
                    and f.endswith(f"referenced_{ref}.fif")
                ...
                if len(fif_files) > 1:
                    ...
                    lfp_file = os.path.join(preprocessed_dir, fif_files[0])        
                
                print(f"Reloading cut and filtered data: {lfp_file}")
                ecog_reref = mne.io.read_raw_fif(lfp_file, preload=True)
                
                print(ecog_reref)
                print("\nChannel names:")
                print(ecog_reref.ch_names)
                ...
                bad_channel_files = [
                    f for f in os.listdir(sub_neural_prep_dir)
                    if (
                        ('bad_channels' in f)
                        and f.endswith('.txt')
                ...
                def pick_most_recent(files):
                    if not files:
                        return None
                    return max(files, key=lambda fn: os.path.getmtime(os.path.join(sub_neural_prep_dir, fn)))
                ...
                bad_channel_path = os.path.join(sub_neural_prep_dir, picked)
                ...
                fif_bads = list(ecog_reref.info.get("bads", []))
                
                # --- NEW: filter to only channels that exist in this Raw ---
                ch_set = set(ecog_reref.ch_names)  # or mne_data.info["ch_names"]
                ...
                # Assign back
                ecog_reref.info["bads"] = merged_bads
                
                new_bads = [ch for ch in existing_bads_valid if ch not in fif_bads]
                
                print(f"Bads already in FIF: {fif_bads}")
                print(f"Bads loaded from file: {existing_bads_valid}")
                print(f"New bads being added: {new_bads}")
                print(f"Final bad list: {merged_bads}")
                
                ecog_reref.info["bads"] = merged_bads
                ecog_reref.save(lfp_file, overwrite=True)
                                
                del ecog_reref, lfp_file
