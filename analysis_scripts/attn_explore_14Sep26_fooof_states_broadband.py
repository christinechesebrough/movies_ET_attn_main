...
# BROADBAND VARIANT (2026-09-15): identical to attn_explore_14Sep26_fooof_states.py
# except the features come from the 1-150 Hz notch-interpolated fits
# (extract_fooof_broadband_interp.py) instead of the 1-57 Hz light fits, HFA is
# added as a periodic band, and outputs go to fooof_states_broadband/.
FOOOF_DIR = f'{MOVIE_DATA}/rolling_fooof_broadband_interp'
FILE_SUFFIX = 'fooof_broadband'
LABELS = f'{MOVIE_DATA}/attention_labels_10s/attention_labels_10s_pooled_explore14Sep26.csv'
PRESENCE = os.environ.get('FOOOF_STATES_PRESENCE', '0') == '1'   # True: Has_theta / Has_alpha as % of windows, no centring -> .../presence/
OUT = f'{MOVIE_DATA}/attn_explore_14Sep26/fooof_states_broadband' + ('/presence' if PRESENCE else '')
FEATURES = ['Has_theta', 'Has_alpha'] if PRESENCE else ['Aperiodic_Exponent', 'Aperiodic_Offset', 'Knee_Freq_Hz', 'Periodic_theta', 'Periodic_alpha', 'Periodic_beta', 'Periodic_gamma', 'Periodic_hfa']
...
def find_file(pat, run, vid):
    n = int(run.split('-')[1])
    hits = [f for f in glob.glob(f'{FOOOF_DIR}/{pat}/{pat}_*{vid}_{FILE_SUFFIX}.csv') if not os.path.basename(f).startswith('._')
